...
public class ZendeskCommand : CommandBase<ServiceRequest, ServiceConfig>
{
    public override string Name => "Zendesk";
    public override string Description => "Integration with Zendesk Support for customer tickets";
    protected override INotificationService NotificationService { get; set; }

    public override List<ToolCall> GetToolDefinitions()
    {
        return ServiceExtensions.GetServiceToolCalls<ZendeskService>();
    }
    ...
    protected override async Task<object> DoWork(ServiceRequest serviceRequest, ServiceConfig config, IEnumerable<ToolCall> availableToolCalls)
    {
        try
        {
            var service = new ZendeskService(Logger);
