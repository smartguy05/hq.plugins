...
public record ServiceConfig : IPluginConfig
{
    ...
    [Tooltip("Maximum characters of markdown to return from read_page before truncating (default 50000).")]
    public int MaxContentLength { get; set; } = 50000;

    [Tooltip("Wait for network to become idle before reading the page (default true). Slower but more reliable on JS-heavy sites.")]
    public bool WaitForNetworkIdle { get; set; } = true;

    [Tooltip("Prepend a small metadata header (title, byline, site, url) to read_page markdown (default true).")]
