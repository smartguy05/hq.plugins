...
- **LinkedIn**: Self-hosted, browser-driven LinkedIn (14 tools) — no external orchestration vendor (replaced Relevance AI; Proxycurl is defunct). Drives a **persistent authenticated Chromium profile** (`LinkedInBrowser` via Playwright `LaunchPersistentContextAsync`, profile dir from `LinkedInPaths`/`PluginDataDirectory`) that retains the httpOnly `li_at` cookie across restarts; the agent never sees the password or cookie. The session is captured once through a one-time **interactive login window** (`IHasFrontend` + `IHasHttpRoutes`; `Endpoints/LinkedInLoginEndpoints` + `ui/index.html` serve a noVNC viewer over Xvfb/x11vnc/websockify managed by `LinkedInLoginSession`). Tools issue authenticated **Voyager API** calls from inside the logged-in page (`Voyager` endpoint catalog — unofficial/drift-prone, isolated for easy fixing; `LinkedInParsing` summarizes, raw JSON always returned). Preserves all 9 original tool names (chats/profile/post/comment/invite/message/InMail) + `react_to_post` + people/company `search_*`/`lookup_*`. Writes are `[SupportsConfirmation]` (EmailService/Stripe pattern) and metered by a per-UTC-day `RateLimitGate` (conservative caps for a real account). ToS-risky by nature — pace conservatively.
